...
namespace GluonCS
{
    public partial class LiveUavPanelSwallow : System.Windows.Forms.UserControl //: LayerPropertiesPanel
    {
        ...
        void updatePanel_Tick(object sender, EventArgs e)
        {
            try
            {
                try
                {
                    if (model.NavigationModel.Blocks["Survey"] > 0)
                    {
                        NavigationInstruction ni = model.GetNavigationInstructionLocal(model.NavigationModel.Blocks["Survey"] + 3);
                        if (ni.opcode != NavigationInstruction.navigation_command.EMPTY)
                            _btn_build_survey.Enabled = true;
                        else
                            _btn_build_survey.Enabled = false;
                    }
                    else
                        _btn_build_survey.Enabled = false;
                }
                ...
                if (model.Serial != null && model.Serial.IsOpen)
                {
                    ...
                    _btn_receive.Enabled = true;
                    _btn_send.Enabled = true;
                }
                else
                {
                    ...
                    _btn_receive.Enabled = false;
                    _btn_send.Enabled = false;
                }
                ...
                Button[] commands = { _btn_Takeoff, _btn_standby, _btn_loiter, _btn_land, _btn_route };
                foreach (Button c in commands)
                {
                    if (c.Tag.ToString() == model.NavigationModel.Commands[model.CurrentNavigationLine].BlockName &&
                        c.BackColor != Color.Yellow)
                        c.BackColor = Color.Yellow;
                    else if (c.BackColor != Color.Transparent && c.Tag.ToString() != model.NavigationModel.Commands[model.CurrentNavigationLine].BlockName)
...
#region model events
        ...
        private void RedrawNavigationtable()   // from GUI thread
        {
                ...
                _btn_standby.Text = "Standby";
                _btn_loiter.Text = "Loiter";
                _btn_Takeoff.Text = "Takeoff";
                _btn_land.Text = "Land";
                _btn_route.Text = "Route";
                lock (model.NavigationModel.Commands)
                {
                    for (int i = 0; i < model.MaxNumberOfNavigationInstructions(); i++)
                    {
                        ...
                        {
                            if (model.NavigationModel.Commands[i].BlockName == "Standby")
                                _btn_standby.Text = "* Standby";
                            else if (model.NavigationModel.Commands[i].BlockName == "Loiter")
                                _btn_loiter.Text = "* Loiter";
                            else if (model.NavigationModel.Commands[i].BlockName == "Takeoff")
                                _btn_Takeoff.Text = "* Takeoff";
                            else if (model.NavigationModel.Commands[i].BlockName == "Descent" || model.NavigationModel.Commands[i].BlockName == "Flare")
                                _btn_land.Text = "* Land";
                            else
                                _btn_route.Text = "* Route";
        ...
        private void distanceTextBox5_Load(object sender, EventArgs e)
        {
        ...
        {
            NavigationInstruction ni = model.GetNavigationInstructionLocal(2);
            Inputbox ib = new Inputbox("Takeoff", "Please enter the altitude the autopilot should climb to at takeoff", "Altitude", "",
                ni.x, 0);
        ...
        private void _btn_params_standby_Click(object sender, EventArgs e)
        {
            NavigationInstruction ni = model.GetNavigationInstructionLocal(4);
            Inputbox ib = new Inputbox("Standby", "Please enter the standby parameters", "Altitude", "Circle radius",
                ni.b, ni.a);
            if (ib.ShowDialog() == DialogResult.OK)
            {
                ni.b = (int) ib.Val1;
        ...
        private void _btn_params_loiter_Click(object sender, EventArgs e)
        {
            NavigationInstruction ni = model.GetNavigationInstructionLocal(8);
            Inputbox ib = new Inputbox("Loiter", "Please enter the loiter parameters", "Circle radius", "",
                ni.a, 0);
            if (ib.ShowDialog() == DialogResult.OK)
            {
                ni.a = (int) ib.Val1;
        ...
        {
            if (_lv_navigation.Location.X == _lv_navigation.Parent.Location.X)
                _lv_navigation.Location = new Point(550, _lv_navigation.Location.Y);
            else
                _lv_navigation.Location = new Point(_lv_navigation.Parent.Location.X, _lv_navigation.Location.Y);
        }

        private void _btn_params_land_Click(object sender, EventArgs e)
        {

        }

        private void _btn_params_route_Click(object sender, EventArgs e)
        {
            Inputbox ib = new Inputbox("Route settings", "Please enter the altitude for your route", "Altitude", "", Properties.Settings.Default.DefaultAltitudeM, 0);
            if (ib.ShowDialog() == DialogResult.OK)
            {
                Properties.Settings.Default.DefaultAltitudeM = ib.Val1;
                Properties.Settings.Default.Save();

                for (int i = model.NavigationModel.Blocks["Route"]; i < model.MaxNumberOfNavigationInstructions(); i++)
                {
        ...
        private void _btn_receive_Click(object sender, EventArgs e)
        {
            _btnNaviRead_Click(sender, e);
        }

        private void _btn_send_Click(object sender, EventArgs e)
        {
            _btnNaviWrite_Click(sender, e);
        }
        ...
        private void _btn_build_survey_Click(object sender, EventArgs e)
        {
            _btnBuildSurvey_Click(this, EventArgs.Empty);
        }
